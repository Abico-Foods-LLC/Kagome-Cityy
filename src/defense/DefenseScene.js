...
const WALK = 5.6, RUN = 9.4, ACCEL = 34, DECEL = 42, AIM_R = 16, BLOB_SPEED = 26, CAM_DIST = 6.2;
...
export class DefenseScene {
  ...
  bindUI() {
    ...
    canvas.addEventListener('mouseup', (e) => { if (e.button === 0) this.firing = false; });
    document.addEventListener('mousemove', (e) => { if (this.entered && document.pointerLockElement === canvas) { this.input.look.dx += e.movementX * 0.0022; this.input.look.dy += e.movementY * 0.0016; } });
  ...
  enter() {
    ...
    toast(this.input.isTouch ? 'Дэлгэц чирж чиглүүл, 🚿 товч дарж бууд' : 'Дэлгэц дээр товшоод хулганаа түгж (Esc гарна) — зүүн товч буудна', 4200, '🚿');
  }
  ...
  shoot() {
    ...
    const { dx, dz } = this.aimDir();
    const n = this.nozzle.getWorldPosition(new T.Vector3());
    this.spawnBlob(n.x, n.y, n.z, dx, dz);
    // Muzzle: цацлагын дусал + гэрэл, recoil, камерын жижиг цохилт
    this.particles.burst(n, 0xc8ff8a, 5, { speed: 5, up: 1.2, size: 0.09, life: 0.25, gravity: 4 });
    this.flash.position.copy(n); this.flashT = 0.07;
    this.character.recoil?.(); this.cam.kick = 0.018;
  ...
  update(dt) {
    ...
    const wantFire = active && !this.over && (this.firing || input.held('interact') || input.held('jump') || input.justPressed('click'));
    ...
    this.cam.kick = Math.max(0, (this.cam.kick || 0) - dt * 0.12);
    const pitch = this.cam.pitch - this.cam.kick, fx = -Math.sin(this.cam.yaw), fz = -Math.cos(this.cam.yaw), rx = -fz, rz = fx;
    const focus = new T.Vector3(P.pos.x + rx * 1.6, 1.7, P.pos.z + rz * 1.6);
    const desired = new T.Vector3(focus.x - fx * Math.cos(pitch) * CAM_DIST, focus.y + Math.sin(pitch) * CAM_DIST, focus.z - fz * Math.cos(pitch) * CAM_DIST);
  ...
  applyEvents() {
    ...
    for (const e of ev) {
      switch (e.t) {
        ...
        case 'hit': { this.hitFlash = 0.12; const p = this.pests.get(e.id); if (p) { p.hitT = 0.2; this.particles.burst(new T.Vector3(e.x, 0.8, e.z), 0x9fe36a, 6, { speed: 2, up: 2, size: 0.12, life: 0.4, gravity: 6 }); } this.audio.tone({ f: 300, f2: 200, type: 'triangle', dur: 0.06, vol: 0.05 }); break; }
