...
const KEYMAP = {
  ...
  Escape: 'pause', KeyP: 'photo', KeyF: 'ball', KeyM: 'map', KeyQ: 'camLeft', KeyR: 'camRight', Tab: 'map', KeyC: 'roll', ControlLeft: 'roll', Digit1: 'emote1', Digit2: 'emote2', Digit3: 'emote3',
};
...
export class Input {
  ...
  bindPointer(canvas, { swipe = false } = {}) {
    ...
    const end = (e) => {
      ...
      if (!active.dragging && performance.now() - active.t < 350 && Math.hypot(e.clientX - active.sx, e.clientY - active.sy) < 8) { this.press('click'); this.release('click'); }
      if (swipe) {
